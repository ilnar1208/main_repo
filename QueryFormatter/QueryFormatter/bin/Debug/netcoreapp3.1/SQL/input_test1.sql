...
f.xLic xLic
  ...
  (
   select
     ...
     , (case 
          when level = 1 then '' 
          when level = 2 then '|' || lpad('_', 3*(level-1), '_')
          else lpad('-', 3*(level-2), '-') || '|' || lpad('_', 3*(level-2), '_')
   ...
   from
     ...
     left join obj$groups og on ogo.object_no = og.no
     left join tfc$dsc$reports tdr on ogo.object_no = tdr.no
     left join dsc$reports tdr_dr on tdr.dsc$report_no = tdr_dr.no
     left join dsc$reports dr on ogo.object_no = dr.no
     left join dsc$queries dq on ogo.object_no = dq.no
   connect by prior
     object_no = group_no
   start with
     group_no = 53684156
...
(select  
  ...
  (select --/*+rule*/      
         cha.from_months_start      
          from  
            dual
          left join (select t from dual) t on f.t = t.k
 full outer join tese y on k.ds= y.fase and tasd.tse = wetqw.sd and tas1d.tse = we1tqw.sd
 right join yivl l on t.k = tasd.la and tasd.tse = wetqw.sd
 join  qwerq l on q.ax = r.sad   
